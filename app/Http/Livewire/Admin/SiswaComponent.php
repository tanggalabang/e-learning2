...
class SiswaComponent extends Component
{
  ...
  public function updated($fields) {
    $this->validateOnly($fields, [
      // 'nis' => 'required|unique:students,student_id,'.$this->student_edit_id.'', //Validation with ignoring own data
      'nama' => 'required',
  ...
  public function storeStudentData() {
    ...
    $this->validate([
      'nis' => 'required', //students = table name
      'nama' => 'required',
      'gender' => 'required',
      'email' => 'required|email',
      'kelas' => 'required',
  ...
  public function render() {
    ...
    $query->when($this->searchTerm != "", function($q) {
      return $q->where('nis', 'like', '%'.$this->searchTerm.'%')
    ...
    $siswas = $query->latest()->paginate(5);;
